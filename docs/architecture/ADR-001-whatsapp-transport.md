...
- **This specific sandbox cannot open a live connection at all** (confirmed by testing, not assumed): this session's outbound network goes through an egress proxy whose own documentation (`/root/.ccr/README.md`) explicitly lists "WebSocket upgrades" as *not supported through the proxy* and instructs agents to report this rather than work around it. `familyos whatsapp:link` was run here and hung indefinitely — no QR, no error — because the WebSocket to WhatsApp's servers never connects or fails, it just stalls. This is an environment restriction, not a Baileys defect: any WhatsApp gateway (Baileys, whatsmeow, whatsapp-web.js) needs the same kind of WebSocket and would hit the same wall here. It is expected to work normally in Termux or any environment with ordinary internet access.
