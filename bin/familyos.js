...
async function main() {
  ...
  if (command === 'personalized-brief') {
    const { runPersonalizedBrief } = require('../src/personalizedBrief');
    await runPersonalizedBrief();
    return;
  }
  ...
  if (command === 'whatsapp:link') {
    ...
    await link({ method: getFlagValue(process.argv, '--method'), phone: getFlagValue(process.argv, '--phone') });
    return;
  }
  ...
  if (command === 'notify') {
    ...
    await runNotify({ to: getFlagValue(process.argv, '--to'), all: process.argv.includes('--all'), strict: process.argv.includes('--strict'), message: getPositionals(process.argv, ['--to']).join(' ') });
    return;
  }
